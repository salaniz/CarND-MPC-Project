...
size_t N = 10;
double dt = 0.1; // the time delay corresponds to one time step
...
// Maximum turning angle delta is 25 degrees (value in radians).
...
// variables in a singular vector. Thus, the following defines
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(int t = 0; t < N; t++) {
      ...
      // error of speed difference
      fg[0] += CppAD::pow(vars[v_start + t] - (ref_v * (1 - 1/max_curvature * curve )), 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double cte = state[4];
  double epsi = state[5];
  double current_delta = state[6];
  double current_a = state[7];
  ...
  // Constrain the first delta to be the one from the previous step
  vars_lowerbound[delta_start] = current_delta;
  vars_upperbound[delta_start] = current_delta;
  ...
  // Constrain the first a to be the one from the previous step
  vars_lowerbound[a_start] = current_a;
  vars_upperbound[a_start] = current_a;
  ...
  // The second actuator pair is returned to account for the 100ms time delay (== to one time step)
